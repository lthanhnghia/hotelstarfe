...
import { getInvoiceStatisticsAll } from '../../../../services/admin/reservation';

const ReservationChart = ({ booking, startDate, endDate }) => {
  ...
  useEffect(() => {
    const handleInvoice = async () => {
      const data = await getInvoiceStatisticsAll();
      setInvoice(data);
    }
    ...
    if (selectedValue) {
      ...
      switch (selectedValue) {
        case "1": // Hôm nay
          ...
          roomsBookedData = labels.map((label, index) => {
            const hourData = filteredBooking.filter(row => new Date(row.bookingDate).getHours() === index);
            return hourData.length > 0 ? hourData.reduce((sum, row) => sum + parseInt(row.totalRoomsBooked), 0) : 0;
          });
          break;
        ...
        case "3": // 7 ngày qua
          ...
          roomsBookedData = labels.map((label, index) => {
            ...
            return dayData.length > 0 ? dayData.reduce((sum, row) => sum + parseInt(row.totalRoomsBooked), 0) : 0;
          });
          break;
        ...
        case "6": // Năm nay
          ...
          roomsBookedData = labels.map((label, index) => {
            ...
            return monthData.length > 0 ? monthData.reduce((sum, row) => sum + parseInt(row.totalRoomsBooked), 0) : 0;
          });
          break;
      ...
      const roomsBookedData = labels.map((label) => {
        ...
        return dayData.length > 0
          ? dayData.reduce((sum, row) => sum + parseInt(row.totalRoomsBooked), 0)
  ...
  const { labels, roomsBookedData } = getFilteredData();
  return (
    <div style={{
        maxWidth: "1100px",
        margin: '20px auto',
